...
sub new
{
 ...
 if ($host =~ m{^/}) {
   # unix socket
   $obj = IO::Socket::UNIX->new($host);
   if (!$obj) {
       carp "failed to create unix socket $host: $!";
       return undef;
   }
 }
 else {
   ...
   if (!$obj) {
       carp "failed to create inet socket $host: $!";
       return undef;
   }
 }
 bless $obj, $type;
...
sub send	  { my $me = shift; $me->_SEND("FROM:" . $me->_addr($_[0])) }
sub send_or_mail  { my $me = shift; $me->_SOML("FROM:" . $me->_addr($_[0])) }
sub send_and_mail { my $me = shift; $me->_SAML("FROM:" . $me->_addr($_[0])) }
...
sub recipient
{
 ...
 foreach $addr (@_) 
  {
    if($lmtp->_RCPT("TO:" . $lmtp->_addr($addr) . $opts)) {
...
The Net::LMTP class is a subclass of Net::Cmd and IO::Socket.
...
This is the constructor for a new Net::LMTP object. C<HOST> is the name of the
remote host to which a LMTP connection is required, or the path to a UNIX
socket where the LMTP server listens.  C<PORT> is the port on which the LMTP
service is running if connecting via TCP.
