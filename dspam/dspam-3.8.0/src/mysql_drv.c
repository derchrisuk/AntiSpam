...
_ds_getall_spamrecords (DSPAM_CTX * CTX, ds_diction_t diction)
{
  ...
  if (s->dbt->mc) {
    /* perform a multi-key get for all tokens in the message */

    /* might not request every item, but alloc anyway for ease */
    char** mc_keys = calloc(diction->items, sizeof(char*));
    ...
    while (ds_term) {
      ...
      ds_term->s.status = 0;
      ds_term->is_memcached = 0;

      snprintf(scratch, sizeof(scratch), "ds:v%d:tok:%d:%llu", s->dbt->mc_key_version, uid, ds_term->key);
      mc_keys[i_key++] = strdup(scratch);

      if (uid != gid) {
        snprintf(scratch, sizeof(scratch), "ds:v%d:tok:%d:%llu", s->dbt->mc_key_version, gid, ds_term->key);
        mc_keys[i_key++] = strdup(scratch);
      }
      ds_term = ds_diction_next(ds_c);
    }
    ...
    size_t* mc_key_lengths = calloc(diction->items, sizeof(size_t));
    for (i_key = 0; i_key < n_keys; i_key++)
      mc_key_lengths[i_key] = strlen(mc_keys[i_key]);

    char return_key[MEMCACHED_MAX_KEY];
    char* return_value;
    size_t return_key_length, return_value_length;
    uint32_t mc_flags;

    mc_rc = memcached_mget(s->dbt->mc, mc_keys, mc_key_lengths, n_keys); /* XXX check mc_rc */

    while ((return_value = memcached_fetch(s->dbt->mc, return_key, &return_key_length, &return_value_length, &mc_flags, &mc_rc))) {
      /* XXX check mc_rc */

      /* key is "rid:token", so first atoi will parse only until the ':' separator */
      int rid = atoi(return_key);

      char* term_str = strchr(return_key, ':');
      if (term_str == NULL)
        continue;
      term_str++;

      token = strtoull(term_str, NULL, 0);

      /* val is "spam:inno", so first atol will parse only until the ':' separator */
      stat.spam_hits = atol(return_value);

      char* inno_str = strchr(return_value, ':');
      if (inno_str == NULL)
        continue;
      inno_str++;

      stat.innocent_hits = atol(inno_str);

      stat.status = 0;

      ds_diction_addstat(diction, token, &stat);

      ds_term = ds_diction_find(diction, token);
      if (ds_term == NULL)
        continue; /* shouldn't happen; we only requested tokens from diction */

      /* may still have to fetch from db if term isn't in both uid and gid dicts */
      if (rid == uid)
        ds_term->is_memcached |= MEMCACHED_USER_TOKEN;
      else
        ds_term->is_memcached |= MEMCACHED_GROUP_TOKEN;
  ...
  ds_c = ds_diction_cursor(diction);
  ds_term = ds_diction_next(ds_c);
  int need_leading_comma = 0;
  while (ds_term)
  {
#ifdef USE_MEMCACHE
    if (s->dbt->mc && ds_term->is_memcached) {
      /* can we avoid looking up this key? */
      if (
        /* without groups, only need user's token to skip query */
        uid == gid
          ||
        /* with groups, need both user and group tokens to skip query */
        (ds_term->is_memcached & MEMCACHED_BOTH_TOKENS) == MEMCACHED_BOTH_TOKENS
      ) {
        ds_term = ds_diction_next(ds_c);
        continue;
      }
  ...
  while ((row = mysql_fetch_row (result)) != NULL)
  {
    ...
    if (s->dbt->mc) {
      ds_term = ds_diction_find(diction, token);
      if (
        /* score and cache if this token was fetched from the database */
        !ds_term->is_memcached ||
        (
          uid != gid && 
          /* when using groups, we might have already scored this token for user
             or group if one was memcached memcached; skip the scoring and memcache
             write for one if we did get from it memcached */
          (
            (rid == uid && !(ds_term->is_memcached & MEMCACHED_USER_TOKEN)) ||
            (rid == gid && !(ds_term->is_memcached & MEMCACHED_GROUP_TOKEN))
          )
        )
      ) {
        /* got this token from the database; memcache and score it */
        char valscratch[64];

        /* compose key and value */
        snprintf(scratch, sizeof(scratch), "ds:v%d:tok:%s:%s", s->dbt->mc_key_version, row[0], row[1]);
        snprintf(valscratch, sizeof(valscratch), "%s:%s", row[2], row[3]);

        /* set the value */
        mc_rc = memcached_add(s->dbt->mc, scratch, strlen(scratch), valscratch, strlen(valscratch), s->dbt->mc_key_expiration, 0);

        ds_diction_addstat(diction, token, &stat);

        if (rid == uid)
          ds_term->is_memcached |= DB_USER_TOKEN;
        else
          ds_term->is_memcached |= DB_GROUP_TOKEN;
      }
    }
    else
#endif
...
#ifdef USE_MEMCACHE
  /* memcache any tokens that weren't in memcached or the database */
  if (s->dbt->mc) {
    ds_c = ds_diction_cursor(diction);
    ds_term = ds_diction_next(ds_c);
    while (ds_term) {
      char* valscratch = "0:0";
      /* only need to add to user/group if token didn't appear in memcached or db */
      if (!(ds_term->is_memcached & (MEMCACHED_USER_TOKEN | DB_USER_TOKEN))) {
        snprintf(scratch, sizeof(scratch), "ds:v%d:tok:%d:%llu", s->dbt->mc_key_version, uid, ds_term->key);
        mc_rc = memcached_add(s->dbt->mc, scratch, strlen(scratch), valscratch, strlen(valscratch), s->dbt->mc_key_expiration, 0);
      }
      if (uid != gid && !(ds_term->is_memcached & (MEMCACHED_GROUP_TOKEN | DB_GROUP_TOKEN))) {
        snprintf(scratch, sizeof(scratch), "ds:v%d:tok:%d:%llu", s->dbt->mc_key_version, gid, ds_term->key);
        mc_rc = memcached_add(s->dbt->mc, scratch, strlen(scratch), valscratch, strlen(valscratch), s->dbt->mc_key_expiration, 0);
      }
      ds_term = ds_diction_next(ds_c);
...
_ds_setall_spamrecords (DSPAM_CTX * CTX, ds_diction_t diction)
{
  ...
  while(ds_term)
  {
    ...
    if (s->dbt->mc) {
      /* compose key and value */
      char valscratch[64];
      snprintf(scratch, sizeof(scratch), "ds:v%d:tok:%d:%llu", s->dbt->mc_key_version, p->pw_uid, ds_term->key);
      snprintf(valscratch, sizeof(valscratch), "%ld:%ld", stat.spam_hits, stat.innocent_hits);

      /* set the value */
      mc_rc = memcached_set(s->dbt->mc, scratch, strlen(scratch), valscratch, strlen(valscratch), s->dbt->mc_key_expiration, 0);
    }
#endif
...
#ifdef USE_MEMCACHE
  /* XXX memcache lookup should go here, but this codepath appears unused by libdspam */
  LOGDEBUG ("_ds_get_spamrecord: unexpected entry in memcache pathway");
#endif
...
#ifdef USE_MEMCACHE
  /* XXX memcache store should go here, but this codepath appears unused by libdspam */
  LOGDEBUG ("_ds_set_spamrecord: unexpected entry in memcache pathway");
#endif
...
_mysql_drv_getpwnam (DSPAM_CTX * CTX, const char *name)
{
...
#ifdef USE_MEMCACHE
  char mc_uid_prefix[24];
  sprintf(mc_uid_prefix, "ds:v%d:uid:", s->dbt->mc_key_version);
  if (s->dbt->mc) {
    char* key = malloc(strlen(name) + strlen(mc_uid_prefix) + 1);
    strcpy(key, mc_uid_prefix);
    strcat(key, name);

    size_t val_length;
    uint32_t mc_flags;
    memcached_return mc_rc;
    char* val = memcached_get(s->dbt->mc, key, strlen(key), &val_length, &mc_flags, &mc_rc);
    free(key);

    if (val != NULL) {
      s->p_getpwnam.pw_uid = strtol(val, NULL, 0);
  ...
  if (s->dbt->mc) {
    ...
    strcpy(key, mc_uid_prefix);
    strcat(key, name);
    mc_rc = memcached_add(s->dbt->mc, key, strlen(key), row[0], strlen(row[0]), s->dbt->mc_key_expiration, 0);
    free(key);
  }
#endif
...
_mysql_drv_setpwnam (DSPAM_CTX * CTX, const char *name)
{
  ...
  snprintf (query, sizeof (query),
            "insert ignore into %s (%s, %s) values(NULL, '%s')",
...
void *_ds_connect (DSPAM_CTX *CTX)
{
  ...
  if (!dbt->mc) {
    char *memcached_servers =
      _ds_read_attribute(CTX->config->attributes, "MySQLMemcachedServers");
    if (memcached_servers != NULL) {
      dbt->mc = memcached_create(NULL);

      /* process the server list */
      memcached_server_st* servers = memcached_servers_parse(memcached_servers);
      memcached_server_push(dbt->mc, servers);
      memcached_server_list_free(servers);

      char *memcached_key_version =
        _ds_read_attribute(CTX->config->attributes, "MySQLMemcachedKeyVersion");
      dbt->mc_key_version = memcached_key_version ? atoi(memcached_key_version) : 0;

      char *memcached_key_expiration =
        _ds_read_attribute(CTX->config->attributes, "MySQLMemcachedKeyExpiration");
      dbt->mc_key_expiration = memcached_key_expiration ? atoi(memcached_key_expiration) : 0;

      char *memcached_timeout =
        _ds_read_attribute(CTX->config->attributes, "MySQLMemcachedTimeout");
      if (memcached_timeout) {
        double timeout = atof(memcached_timeout);

        // connect/poll timeout is in milliseconds
        memcached_behavior_set(dbt->mc, MEMCACHED_BEHAVIOR_CONNECT_TIMEOUT, timeout*1000);
        memcached_behavior_set(dbt->mc, MEMCACHED_BEHAVIOR_POLL_TIMEOUT,    timeout*1000);

        // send/receive timeout is in microseconds
        memcached_behavior_set(dbt->mc, MEMCACHED_BEHAVIOR_SND_TIMEOUT, timeout*1000000);
        memcached_behavior_set(dbt->mc, MEMCACHED_BEHAVIOR_RCV_TIMEOUT, timeout*1000000);

      }

      // retry timeout is in seconds
      memcached_behavior_set(dbt->mc, MEMCACHED_BEHAVIOR_RETRY_TIMEOUT, 20);
    }
  }
#endif
